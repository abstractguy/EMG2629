...
--]]
	...
	local page  = node("expert", "configuration", "management", "remote", "tr069")
	page.target = call("action_remote_tr069")
	page.title  = "Remote MGMT TR069"
	page.order  = 931	
		...
		if connection_type == "Bridge" then
			...
			elseif WAN_MAC_Clone == "1" then
				...
				if sw==1 then
					...
					local url = luci.dispatcher.build_url("expert","configuration","network","wan0")		
			...
			-- IPoE/ v4
				if (connection_IPmode == "IPv4_Only") or (connection_IPmode == "Dual_Stack") then	
					...
					if WAN_IP_Auto == "1" then
						...
						uci:set("network","wan1","isp_gw",Fixed_staticGateway)
					end
			...
			if connection_type == "PPPOE" then
				...
				if (connection_IPmode == "IPv4_Only") or (connection_IPmode == "Dual_Stack") then	
					...
					-- IPoE/ v4/ STATIC
						...
						uci:set("network","wan2","isp_gw",Fixed_staticGateway)
					end
			...
			-- IPoE
				...
				if (connection_IPmode == "IPv4_Only") or (connection_IPmode == "Dual_Stack") then	
					...
					-- IPoE/ v4/ STATIC
						...
						uci:set("network","wan3","isp_gw",Fixed_staticGateway)
					end
			...
			if connection_type == "PPPOE" then
				...
				if (connection_IPmode == "IPv4_Only") or (connection_IPmode == "Dual_Stack") then	
					...
					-- IPoE/ v4/ STATIC
						...
						uci:set("network","wan4","isp_gw",Fixed_staticGateway)
					end
...
function action_remote_tr069()
	...
	local cafile = "ACS-CA.pem"
	local file
	luci.http.setfilehandler(
		function(meta, chunk, eof)
			if not file and chunk and #chunk > 0 then
				file = io.open("/tmp/" .. cafile, "w")
			end
	...
	if upload then
		local size = nixio.fs.stat("/tmp/" .. cafile).size
		if size > 51200 then
			luci.template.render("expert_configuration/remote_tr069", {errmsg=1})
			sys.exec("rm /tmp/".. cafile)
			return
		else
			sys.exec("cp /tmp/" .. cafile .. " /etc/cert/ca/" .. cafile)
			sys.exec("rm /tmp/".. cafile)			
		end
	...
	if apply then
		local tr69cInform = luci.http.formvalue("tr69cInform")
		local tr69cInformInterval = luci.http.formvalue("tr69cInformInterval")
		local tr69cAcsURL = luci.http.formvalue("tr69cAcsURL")
		local tr69cAcsUser = luci.http.formvalue("tr69cAcsUser")
		local tr69cAcsPwd = luci.http.formvalue("tr69cAcsPwd")
		local tr69cConnReqPort = luci.http.formvalue("tr69cConnReqPort")
		local tr69cConnReqUser = luci.http.formvalue("tr69cConnReqUser")
		local tr69cConnReqPwd = luci.http.formvalue("tr69cConnReqPwd")
		local tr69cInterface = luci.http.formvalue("tr69cInterface")
		local tr69cInformSource = luci.http.formvalue("tr69cInformSource")
		
		uci:set("tr069", "main", "tr69cInform", tr69cInform);
		uci:set("tr069", "main", "tr69cInformInterval", tr69cInformInterval);
		uci:set("tr069", "main", "tr69cAcsURL", tr69cAcsURL);
		uci:set("tr069", "main", "tr69cAcsUser", tr69cAcsUser);
		uci:set("tr069", "main", "tr69cAcsPwd", tr69cAcsPwd);
		uci:set("tr069", "main", "tr69cConnReqPort", tr69cConnReqPort);
		uci:set("tr069", "main", "tr69cConnReqUser", tr69cConnReqUser);
		uci:set("tr069", "main", "tr69cConnReqPwd", tr69cConnReqPwd);
		uci:set("tr069", "main", "tr69cInformSource", tr69cInformSource);

		if tr69cInterface then
			uci:set("tr069", "main", "tr69cInterface", tr69cInterface);
		end
	...
	luci.template.render("expert_configuration/remote_tr069")
end
